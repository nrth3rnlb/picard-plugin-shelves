...
class ShelfProcessors:
    ...
    def _apply_workflow_transition(shelf: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            workflow_stage_1 = config.setting[ShelfConstants.CONFIG_WORKFLOW_STAGE_1_SHELVES_KEY  # type: ignore
            ]
            workflow_stage_2 = config.setting[ShelfConstants.CONFIG_WORKFLOW_STAGE_2_SHELVES_KEY  # type: ignore
            ]
            ...
            if apply_transition and workflow_stage_2:
                ...
                if shelf != destination_shelf:
                    log.debug(
                        "Applying workflow transition: '%s' -> '%s'", shelf, destination_shelf, )
                    return destination_shelf
        except KeyError as e:
            log.debug(
                "Workflow configuration key not found (%s), skipping transition.", e, )
    ...
    def file_post_removal_from_track_processor(_track: Any, file: Any) -> None:
        ...
        """
        log.debug(
            "(file_post_removal_from_track_processor) Processing file: %s", file.filename, )
    ...
    def file_post_addition_to_track_processor(track: Optional[Any], file: Any) -> None:
        ...
        try:
            ...
            is_manual_shelf = (isinstance(shelf_from_tag, str) and ShelfConstants.MANUAL_SHELF_SUFFIX in shelf_from_tag)
            ...
            if shelf_from_path and was_explicitly_found:
                ...
                log.debug(
                    "Priority 1: Physical location in specific shelf '%s' wins.", shelf_name, )
            ...
            # PRIORITY 2: Persistent manual tag
                ...
                log.debug(
                    "Priority 2: Persisted manual tag '%s' wins.", shelf_tag, )
            ...
            # PRIORITY 3: Standard logic
                ...
                shelf_tag = shelf_name
                log.debug(
                    "Priority 3: Default logic. Path shelf '%s', final shelf '%s'.", shelf_from_path, shelf_name, )
                ...
                if album_id:
                    ...
                    if was_explicitly_found or is_manual_shelf:
                        ShelfManager().set_album_shelf(
                            album_id=album_id, shelf=shelf_name, lock=True, )
                    else:
    ...
    def set_shelf_in_metadata(
            _album: Any, metadata: Dict[str, Any], _track: Any, _release: Any, ) -> None:
        """
        ...
        if shelf_name is not None:
            log.debug(
                "Setting shelf '%s' on track from source '%s'", shelf_name, source, )
