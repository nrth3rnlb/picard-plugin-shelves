"""
Tests for the Processors class and its associated strategies.
...
def test_known_identical_names_strategy():
    ...
    assert isinstance(strategy, expected_strategy), (
        f"Expected {expected_strategy.__name__} but got {strategy.__class__.__name__}"
    )


def test_known_name_from_path_differs_from_tag():
    # Arrange
    file_mock = MagicMock(spec=File)
    file_mock.filename = "/music/ShelfA/artist/album/track.mp3"
...
def test_strategy_manual_unset():
    """
