...
class SetShelfActionTest(unittest.TestCase):
    ...
    def test_callback(self, mock_config, mock_dialog_cls, mock_get_configured_shelves, mock_validate):
        """Callback test"""
        # Arrange
        self.actions._set_shelf_recursive = MagicMock()
        mock_config.setting = self.config_setting
        mock_get_configured_shelves.return_value = self.config_setting[ShelfConstants.CONFIG_KNOWN_SHELVES_KEY]
...
class SetShelfDialogTest(unittest.TestCase):
    ...
    def test_ask_for_shelf_name(self, mock_config, mock_get_configured_shelves):
        ...
        self.dialog.shelf_combo.currentText.return_value = "NewShelf"  # Mock currentText to return the expected value
...
class DetermineShelfActionTest(unittest.TestCase):
    ...
    def test_callback(
        ...
        # Assert
        mock_get_shelf_from_path.assert_called_once_with(file_path=file_path, base_path=None)
        self.assertEqual(file_mock.metadata[ShelfConstants.TAG_KEY], "Standard")
